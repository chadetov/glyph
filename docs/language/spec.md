...
- **D30. String-literal-union types (`type Tier = "free" | "pro"`).** A union of string literals is a type, distinct from a D8 tagged union (whose members are named constructors). It emits as the TypeScript literal union (`"free" | "pro"`), so `tsc` enforces the narrowed type at every use, and a record field of this type gets a **runtime membership check** in its descriptor (the value must be one of the literals, not merely a `string`) — the leaf-value check a bare `string` cannot express. This is what lets `glyph gen dts`/`openapi`/`zod` materialize a string enum faithfully instead of narrowing it to `string`. **v1 scope:** single-line, pure literal unions (`"a" | "b"`); mixing a literal with a named type (`"a" | number`), and the multi-line `|`-per-line form, are not supported. Glyph's own checker currently lowers the type to `string` (permissive at the Glyph level; `tsc` and the descriptor do the enforcement), so Glyph-native exhaustiveness over the literal set (a `match` that covers every literal without an `else`, cf. E0218) is a follow-up. *[verifiability — the boundary validates enum membership at runtime; greppability — the allowed set is spelled at the declaration site]*
