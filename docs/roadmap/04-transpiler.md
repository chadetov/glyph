...
Status: **✅ Shipped.** The transpiler is complete and driving `glyph build`. The strategic design decisions below are the long-form rationale kept for the record; for the progress history, see `docs/implementation-plan.md` and git history.

## Implementation status

The whole pipeline is shipped and running on `main`:

- **Lexer + AST + Pratt parser** — every D-decision parses (JSX sub-grammar D6 incl. prop spread, template literals D22, annotations D27, match D3, tagged unions D8, patterns D9, the `extern_ts` escape hatch D29, string-literal unions D30, `typeof` type queries D32); checked-in `insta` snapshots.
- **Resolver + salsa pipeline** — name resolution (intra- and cross-module via `import M { N }` verification), the `Ty`/`TypeMap` side table, and a full salsa graph (`parse → module_symbols → resolve → decl_ast → resolved_decl → decl_ty → type_map → project_exports → import_diagnostics`) with per-declaration input slicing, source-byte canonical fingerprints, and cross-file auto-invalidation.
- **Typechecker** — match exhaustiveness (tagged unions, arrays, bool, number/string, string-literal unions), the `?` operator rule, call/`await` synthesis, generic instantiation, `owned` single-consumption (D25), and runtime descriptors (`is`/`parse`/`schema`) with deep, generic, membership, and integer validation.
...
- **The emitter consumes semantic facts; it does not rediscover them.** The "dumb visitor" decision above was about not building an IR, not about the emitter reasoning on its own. What a name resolves to, whether a declaration is a record, union, alias, resource or refinement, a union's variants and their payload types, a record's fields, whether a name is imported and from where, what type an expression has: the resolver and the typechecker compute these once, and the emitter reads them. An emitter that walks the AST to answer the same question a second time can answer it differently, and every such disagreement is a program the checker accepts and the emitted TypeScript gets wrong, or the reverse. The ledger has paid for this more than once: G147 (the emitter's own variant-list rule missed an imported payload the checker knew), G207 (the emitter grew a second alias walker for the `where` refusal), G212 (the ownership pass read a name the rest of the checker had already resolved). Where a fact the emitter needs is not computed upstream, it is added upstream, as a field on the context the driver passes in or a query in `glyph-db`, never re-derived downstream. The inventory of current violations and the plan to remove them is the 0.1.120 lane in `releases.md`.
