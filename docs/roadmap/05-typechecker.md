...
- **Nested-pattern exhaustiveness** (day 25) — done for constructor nesting. A variant reached only through a single payload sub-pattern has that payload checked recursively, so `match r { Ok(Some(x)) => .., Err(e) => .. }` over `Result<Option<T>, E>` now flags the missing `Ok(None)`. Arbitrary depth; works for module-local unions and prelude `Result`/`Option`. Since 0.1.91 a module-local union applied through `Ty::App` (`Tree<K>`) is checked the same as its bare form, both at the top level and one level down inside a payload: `resolve_named_union` unwraps the application, so the variant set and the payload type it feeds the recursion do not depend on the union's arity. Object/array/literal nested payloads are still conservatively treated as fully covered (no false positives). An imported union goes through `check_imported_union_coverage`, and since 0.1.95 the gate that reaches it reads `union_base(scrutinee_ty)`, so an applied imported union (`Tree<string>`, or an open `Tree<K>`) is counted where it used to be skipped entirely (G148 closed the half of G142 that was left). The check itself is still shallower than the module-local one: it counts the outer variants and does not descend into a payload, generic or not (`docs/dogfooding-gaps.md` G143).
- **`match` expression typing** (0.1.45) — a `match` takes its arms' type through an equality join: an arm ending in `return`/`break`/`continue` diverges and contributes nothing, every other arm contributes its value type, and disagreement or an undecidable contributor leaves `Ty::Unknown` (the old behavior for every `match`). Arm-payload binding covers prelude unions too, so `Ok(v)` over a `Result` binds `v`. Because Glyph has no `if`, this is what keeps a branched value checkable by Glyph instead of by `tsc` (D3).
- **`std/http` response constructors** (0.1.46) — `stdlib_fn_ty` models `html`, `redirect`, and `with_header` as returning `http.Response`. They are the first modeled stdlib functions that do not return a `Result`, and without them a handler's `Ok(http.html(...))` types as unknown and is checked only by `tsc` on the emitted TypeScript.
- **Cross-module type identity** (0.1.58) — a type declared in a sibling module lowers to `Ty::Imported { module, name }`, keyed on the source module's registry path and the name that module declares, so the named, namespace-qualified and aliased import spellings all produce the same type. It carries no symbol id: a foreign module's ids index an unrelated symbol in the consumer's table. Nothing is expanded at lowering; the declaration is fetched on demand by `DeclTyResolver::imported_type_decl`, answered from a tracked `exported_type(db, file, decl_idx)` query that lowers the declaration on the *source* side (the consumer cannot, since `Lowerer::lower` resolves through spans belonging to the declaring file). That gives an imported record a field set, so a typo'd field is `E0210` naming the record's own type and `for i, x` over one of its array fields binds a number. Assignability across the boundary stays permissive (`ty_is_decidable` is false for the new variant) and an `interface`'s member set does not cross yet.
...
- **Fuller bidirectional checker** — broader assignability beyond primitives, and a real unifier (generic instantiation today is call-site substitution, not full unification). The general Maranget matrix over products of independent refutable columns waits on it too. (Exhaustiveness for generic module-local unions was listed here and did not need the unifier: 0.1.91 got it by unwrapping the application in the resolver both callers already shared.) The operand-side `?` `E`-match is now done (it compares the decidable error types directly).
- **Runtime descriptors** (Q8) — non-generic `record` AND non-generic tagged `union` declarations emit a runtime descriptor in the generated TS: an `is` type guard plus a self-contained `parse` (validates an `unknown` into an inlined `Result` shape, so no `std/result` import is needed) and a `T.schema` member. `is TypeName` match patterns lower to a check that calls the descriptor's `is`, so the descriptor is both emitted and consumed. The guard is shallow (records check each field's presence/`typeof`; unions check the discriminant tag) — the documented v1 validation scope. Generic records/unions and deep/recursive field validation shipped after this was written; a generic descriptor's `parse` takes one runtime checker per type parameter, supplied at the call site (`Page.parse<User>(v, (cv) => User.is(cv))`). Remaining: the typechecker knows the signature of `T.parse` for the **non-generic** case only (`Result<T, Array<Issue>>`, added in 0.1.45), so a generic `parse` still types as unknown and only `tsc` checks what you do with its result.
