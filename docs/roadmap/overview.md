...
Current near-term work (per `docs/implementation-plan.md`): the typechecker's substep 5a is largely complete through day 26 (exhaustiveness, `owned`, inference pieces). **Week-4 TS emission started on day 27**: the core AST→TS visitor ships and `glyph build` writes a `.ts` per clean module. Forthcoming emission days: `match` → `switch`, the `?` operator's inlined unwrap, tagged unions → discriminated unions + Q8 runtime descriptors, and `component` + JSX lowering. A fuller unifier (which also unblocks exhaustiveness for generic module-local unions) remains on the typechecker side. Active open questions are tracked in `docs/open-questions.md`.
