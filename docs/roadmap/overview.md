...
Current near-term work (per `docs/implementation-plan.md`): the typechecker's `owned` (D25), `?`/exhaustiveness/inference pieces, recursive nested-pattern exhaustiveness, and array-match exhaustiveness shipped through day 26. A fuller unifier (which also unblocks exhaustiveness for generic module-local unions) remains. Runtime descriptors (Q8) are the next major item but are coupled to phase 1 week 4 (TS emission), which is the natural next phase. Active open questions are tracked in `docs/open-questions.md`.
