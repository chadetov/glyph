...
| 4 | Transpiler to TypeScript (originally 4–6 weeks; revised 6–8) | 🟨 **In progress, 14 days shipped.** Phase 1 weeks 1–2 done: lexer + Pratt parser + AST (all 27 D-decisions, all 4 examples parse), name resolution, module graph, type representation, full salsa-tracked incremental pipeline, `glyph build` CLI, ariadne diagnostics. Week 3 started (typechecker substep 5a — see step 5). TS emission (week 4) still ahead. **179 workspace tests pass.** | `docs/roadmap/04-transpiler.md`, `docs/implementation-plan.md` |
| 5 | Typechecker (originally 4–6 weeks → revised ~9 weeks) | 🟨 **Substep 5a started.** First semantic check shipped: Maranget-style variant-set exhaustiveness for `match` over user-defined tagged unions (day 14). Bidirectional checker, `?` propagation typing, `owned` single-consumption, runtime descriptors still ahead. Q1 resolved (mapped types → v1.1, substep 5b deferred). | `docs/roadmap/05-typechecker.md`, `docs/implementation-plan.md` |
...
Current near-term work (per `docs/implementation-plan.md`): week-3 typechecker pieces — bidirectional checker, `?` propagation typing, `owned` single-consumption analysis, runtime descriptors. Then phase 1 week 4 (TS emission). Active open questions are tracked in `docs/open-questions.md`.
