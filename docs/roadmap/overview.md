...
Current near-term work (per `docs/implementation-plan.md`): the typechecker's `owned` (D25), `?`/exhaustiveness/inference pieces, and recursive nested-pattern exhaustiveness shipped through day 25. Remaining before TS emission: array/object pattern exhaustiveness and a fuller unifier. Runtime descriptors (Q8) are the next major item but are coupled to phase 1 week 4 (TS emission), which is the natural next phase. Active open questions are tracked in `docs/open-questions.md`.
