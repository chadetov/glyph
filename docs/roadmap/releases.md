...
- **`glyph --explain <CODE> --json`.** The code, its title, the long explanation, the `help`, the docs URL, and a counter-example taken from `tests/negative/`: the wrong program's source, and the diagnostic this compiler draws for it, built when the question is asked rather than stored. `corrected` is `null` beside a sentence saying why: the corpus pairs no repaired program with a case, and a fix invented at answer time would compile nowhere and be checked by nothing. A code the corpus does not cover gets `counter_example: null` and the matching sentence, so the corpus claims nothing about the codes it does not hold
...
- **The measurement.** The suite is 1697 tests, from 1625; the exact-or-absent corpus is 33 invariants, from 31; the gap ledger is 236 entries
