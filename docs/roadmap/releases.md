...
- G231: an entity identity for a prelude declaration, so `glyph_symbol`, `glyph fix` and `glyph check --agent` can key `Result` and `Option`. **This is a decision before it is a fix, and it should be made rather than worked around.** The compiler holds the shape: the checker knows `Err` carries the error type, `glyph_variants` reports the site's missing variants, and `glyph-resolver/src/prelude.rs` declares both unions. What is missing is a key, and `glyph_symbol` refuses to invent one today by design, because `module::name` names a module and no project has a module that declares `Result`. The options are a reserved module segment that no project file can spell, a `kind` on the identity that says the declaration is the prelude's rather than a project's, or leaving prelude declarations unkeyed and giving the tools that need a shape a second, non-identity way to ask for one. Whichever is taken decides what `cause`, `entity` and every `glyph_query` argument mean for a name the project did not declare, so it is not a choice to make inside a `glyph fix` rule. Until it is made, `glyph fix` declines on the two unions a real program matches on most often, which is the rule repairing nothing for `match r { Ok(v) => v, }`
