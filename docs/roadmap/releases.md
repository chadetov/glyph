...
- **G222, the surface.** `glyph llms --json` emits six sections and nothing in it is written by hand. `diagnostics`: 61 codes, each with the catalogue sentence, the one-line fix, the whole `--explain` text, the help and note read off a real diagnostic, and a wrong program from `tests/negative/` compiled at the moment you ask. `prelude`: 24 names out of `build_prelude`. `stdlib`: 36 modules and 327 exports, 100 with a signature read out of the checker's `stdlib_*_fn_ty` tables and rendered by `display_ty`, 227 saying in a `signature_absent` line that the checker models none. `decisions`: 47 parsed out of the spec's own list. `tools`: the 11 `tool_specs()` serves, each with its `glyph query` verb and its full manual. The whole document builds in about a second
...
- **G234, and a common shape that got more accurate.** `glyph_impact`'s `change_signature_type` table sent a string-literal union to `Other` on both sides, so six cells said the checker has no rule for a pairing G230 had given it one for, while `glyph_assignable` on the same pairing answered `COMPATIBLE`. The union is its own kind on both sides now, however it is spelled, and its cells run the same `assignability` call `glyph_assignable` makes, the way the container and record cells have since G228, so the two tools cannot answer differently about one pairing. Six cells are asserted in both verdict directions against the tools' own answers rather than against fixed strings. One visible consequence is worth naming rather than burying: a written literal is no longer a primitive argument, it is a one-literal union, so `f("x")` against `f(s: string)` reports argument 1 `SAFE` where 0.1.122 reported `WILL_FAIL`. That is a common shape and the new answer is the accurate one, decided by the relation instead of by a sentence about two kinds
- **What it does not do.** No retrieval layer, no embeddings, no chat surface, and no documentation written by hand about the compiler where the compiler can be asked. `glyph llms --json` is a document you fetch once, not a service. 21 of the 61 codes have no wrong program in the corpus, and the answer says so rather than inventing one. A stdlib export the checker models no declaration for still answers `kind: null` with the reason. An object literal still has no type of its own, so `let g: string = { x: 1 }` is silent under `--no-tsc` (G232), and the editor's hover still answers single-file where the tool answers from the project (G235)
- **The measurement.** The suite is 1740 tests, from 1714; the exact-or-absent corpus is 34 invariants, from 33; the gap ledger is 238 entries, 215 fixed and 5 open. The positive string-literal-union corpus is nineteen programs, from thirteen, each run under the published 0.1.122 with `tsc` in the loop before it was added. The 31 apps at their own roots and `examples/` answer with identical exit codes and identical diagnostic-code lists under both binaries
