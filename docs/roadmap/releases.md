...
- **What it does not do.** `glyph fix` does not guess, and the list of what it therefore declines is longer than the list of what it repairs. It cannot repair a match over a prelude union, because no identity keys `Result` or `Option` and the rule needs the payload shapes from `glyph_symbol` (G231). It cannot repair a match over a union declared in another module whose missing variants are not already named in the import list, because the rule verifies its own work by resolving the candidate file alone (G236). `corrected` on `--explain --json` is null. `cause` on E0211 is null. An object literal still has no type, so `let g: string = { x: 1 }` is still silent under `--no-tsc` (G232), and the impact table still understates the checker for a string-literal union on either side (G234)
...
**G236. `glyph fix` declines the E0200 repair over an imported union.** The rule
renders the arms and then verifies its own work by parsing and resolving the
candidate file alone, so a variant declared in another module and not already
named in the import list reads as an unresolved name and the rule declines. That
is the ordinary shape, and the unused-import rule runs first and strips the very
names that would have made the guard pass. Fixing it means either extending the
import list as part of the repair or verifying the candidate against the project,
and the second is the same single-file-versus-project split as G235 and G219.
Parked here rather than put in 0.1.123 because it is a defect in what 0.1.122
shipped and the orchestrator may want it there instead; it was found while
writing the release notes, after the scope was set.
