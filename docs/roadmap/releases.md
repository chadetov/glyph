...
- G91: **decided, and not the way it was proposed.** Loosening `Option<T>` to accept `null` and bare values was rejected: `@open type Meta = { id: int }` with `{"m": {"tag": "None", "id": 7}}` decodes two ways, both valid against the declared types, and `type Tagged = { tag: string, value: int }` under `Option<Tagged>` has a valid document only a backtracking decoder accepts. The wire format of a field would depend on a property of a type declared in another file, which greppability forbids. Instead a prelude `Nullable<T>`: the same Glyph type as `Option<T>` (so `match` on `Some`/`None` and exhaustiveness are unchanged), emitted as `T | null`, with a descriptor that decodes `null` or absent to `None` and anything else through `T`'s check into `Some`; `T` may not itself be `Nullable` or `Option`, one error. `Option<T>` is untouched and the release is not breaking on this account. `gen dts` and `gen openapi` route a `nullable` schema field to it instead of collapsing it to an optional field. The entry's premise is narrowed too: an optional field of the base type has accepted `null` since 0.1.68, so what is broken is having `null` tolerance and a matchable `Option` in one field
