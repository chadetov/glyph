...
**0.1.114 — Landed on main · The incremental debt, in the order it actually binds**
...
**0.1.115 — Next · The language server stops running a second compiler**
...
**Built, measured, and held rather than shipped.** The overlay works and its reuse is total: a second request over an unchanged buffer executes zero queries, proved by a salsa event log. An editor burst falls from 61.81ms to 37.11ms and a repeated workspace-wide references from 13.47ms to 1.31ms.

**But the bare keystroke rose from 15.60ms to 33.39ms, and that is a finding rather than a tuning problem.** `glyph_db::type_map` costs 21.21ms where `assign_types` over the same text costs 4.20ms, and 16.24ms of the gap is the per-declaration layer `typed_file` drives: `decl_ast` clones every declaration and copies its source bytes, `resolved_decl` clones a sliced `ResolvedModule` per declaration. That layer pays for itself in a build, where its memos are read across files and revisions. On a keystroke it is paid in full to save the 4.2ms of assignment it wraps.

**Decision: fix the fill cost first, then ship the overlay unchanged.** Shipping as built would double squiggle latency and raise the keystroke's growth exponent from n^1.47 to n^1.74, and a release whose purpose is to make this fast enough for a human does not get to make the felt path slower. Keeping types off the database instead would leave the server re-running a compiler stage per request, which is the thing this release exists to stop, and would bury the cost rather than surface it. With the fill cost fixed the measurement says the keystroke returns to about 15.4ms and the burst falls to about 16ms, a 74% cut with no regression anywhere.
