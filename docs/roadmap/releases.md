...
- G128: **decided, option (b) with three corrections.** A default deadline of 30 000 ms, set in `request`'s bounds and in `head`'s literal so all six verbs (`get`, `post`, `put`, `patch`, `del`, `head`) are covered, not only `fetch_of`. An explicit `timeout_ms: 0` in a `Fetch` sent through `send` keeps meaning no timeout, so the change is not breaking and the opt-out is one greppable literal at the call site. `http` gains the `MAX_TIMEOUT_MS` guard `tls` already has, since a deadline past `setTimeout`'s clamp currently aborts in one millisecond and blames a deadline that never elapsed. The number is written in the `Fetch` doc comment and beside the signature in the stdlib reference, where the reader is. G127's "a bound nobody passes is a bound nobody has" argued against optional, and a default is a bound somebody has and nobody chose: visible and recoverable as `kind: "timeout"`, where the absent bound hangs. undici's 300 s ceiling is node-only and absent in the bundled realm this repo supports, so it does not make the default cosmetic
