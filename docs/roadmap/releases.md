...
- `tools/list` stops costing what it costs: tool descriptions carry the contract in one paragraph and point at `glyph llms --json` for the rest, measured before and after. The baseline is 0.1.122, measured over `glyph mcp` stdio on the release tree: eleven tools, 31,446 bytes of description and a 45,509-byte `tools/list` result. Measured the same way in the same sitting, the published 0.1.121 is nine tools, 27,526 and 38,928, so the release that closed the G221 tool set grew the reply by 17% and the lane's target moves with it. The earlier recorded figures for 0.1.121 and 0.1.120 (27,386 / 39,077 and 18,854 / 28,685) came from a different measurement and are not comparable to these; the method here is the sum of the `description` bytes over the tools in the reply, and the byte length of the `result` object serialized compactly
- G233, **done**: an imported string-literal union printed as its literal set while an imported alias of the same declaration printed as its name, so one declaration had two spellings in a diagnostic and neither was what a local declaration printed. It lowers to the `Ty::Imported` every other imported declaration lowers to now, and the literal set is read from that declaration through one accessor that every consumer goes through: assignability, match exhaustiveness, `glyph_symbol`, `glyph_assignable`, and the emitter's narrowing assertion and comparison lowering. ``expected `Mode` `` under both import spellings, ``expected `Alias` `` for the alias, which is what the same two declarations print locally. The lowerer reaches across no module boundary at all now, so `Lowerer::with_imports` and the per-shape query behind it are gone
- G237, **done**: literal-typed expressions, so a string-literal union reached through a generic argument or a record field is decided rather than fenced. A written string literal has the one-literal type it spells, an array literal joins its elements' types, and a fresh literal widens to `string` at a binding that infers its type from the value: a `let` with no annotation and a `for` element binding, both of which widen in the emitted TypeScript too. A `const` keeps its literal, as its emitted `const` does, and a literal type reached through a name is not fresh and does not widen. Every row of that rule was established by running `tsc --strict` on the equivalent TypeScript. With the literal on the value, 0.1.122's generic-argument fence is gone and a written array or object literal is checked element by element and field by field, so a refusal names the offending literal and underlines it. Eight programs `tsc` refuses and 0.1.122 accepted are now `E0204` or `E0211`; the four correct programs the fence protected are accepted by the literal-set rule; the 31 apps and `examples/` answer identically under both binaries; the positive corpus is nineteen programs, from thirteen. The one pairing still declined is a `Record`'s key, where TypeScript's index signature accepts a `string` key against a literal-union key in both directions
- G234, **done**: `glyph_impact`'s `change_signature_type` table sent a string-literal union to `Other` on both sides, so six cells said the checker had no rule for a pairing G230 gave it one for while `glyph_assignable` on the same pairing said `COMPATIBLE`. The union is its own kind on both sides now, however it is spelled, and its cells run `assignability` on the two types the site holds the way the container and record cells have since G228, because the rule reaches through a name, an import and a `Nullable` and no sentence about two kinds is true of it. The review's own project answers `SAFE` where it answered `UNDETERMINED`, and the two tools agree on six cells asserted in both verdict directions
- G231, decided 2026-09-15: **a prelude declaration is keyed by the stdlib module that declares it**, so the identity of the prelude `Result` is `std/result::Result`, `Option` is `std/option::Option`, `Nullable` is `std/nullable::Nullable`, and the same rule gives `fs.ErrorKind` its address `std/fs::ErrorKind`. Not a new reserved segment and not a kind beside the name. The key already exists in three places the agent surface throws away: the resolver registers `std/result` with `Result`, `Ok` and `Err`; the emitter writes `import { Ok, type Result } from "./.glyph-runtime/std/result"` for a program that never imported it; and `import std/result { Result }` resolves today. The identity is discarded, not invented, and the comment in `glyph-lsp/src/diagnostic.rs` saying a key for `Result` would name a module no project has is false. A `kind` on the identity was refused because the union entity already carries `kind: "builtin"` beside a bare name and it buys the agent no next call; because the axis is not binary (`fs.ErrorKind` is stdlib, not prelude); because `kind` is already used twice on those objects; and because the whole surface rests on forwarding one flat string from a diagnostic into the next query (three tools take an entity argument, 27 emission sites in `mcp.rs`, the exact-or-absent gate and the committed generation records all key on it). A new segment such as `<prelude>` was refused because it would give one declaration two identities, the agent's and the emitter's, which is the G75 rule broken on purpose. The surfaces that change: `UnionRef::Builtin` and `DiagnosticDecl::Builtin` carry the module key so `cause`, `union.module` and `union.declaration` stop being null; `describe_symbol` gains a stdlib branch built from `StdlibStubs` and the checker's own tables, with `path` and `range` absent with reasons since the stdlib has no Glyph source; `glyph_variants`, `glyph_exports` on a `std/` module and position-addressed `glyph_symbol` on a prelude name answer instead of refusing; `glyph fix` gains one arm so a prelude re-export counts as in scope without an import; and the residue that no stdlib module declares (`Array`, `Record`, `Schema`, `Component`, `Issue`, `par`, `print`, `assert`, `infer_output`) stays unkeyed under `NOT_A_DECLARATION` with a stated reason each, rather than getting an invented `std/prelude`. The collision guard is G238 below, and it lands first. `union.kind` stays `builtin` because it still answers whether this project can add a variant. The one thing that reopens the decision: the prelude `Result` ever becoming a declaration distinct from `std/result`'s. The review is Linus 67
- G238: a project file may declare `module std/io` (or any `std/` or `extern/` path); it compiles clean and is silently unreachable, because an import of that path resolves to the stdlib stub and the project file is never read. A `module` declaration whose first segment is `std` or `extern` becomes an error with its own code, which is the collision guard G231's identities rest on and a dead-code hole on its own. D15 is corrected to say both prefixes are reserved
