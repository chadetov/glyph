...
- G27: **premise to re-check first.** The entry records both halves fixed and is still marked half fixed; establish what, if anything, still leaks before touching code
- G147: a lowercase nullary variant of an imported payload union does not dispatch. `EmitContext` gains a per-module registry of variant names, the sibling of `record_payload_variants`, so the variant-list half of the nested-arm rule answers across the boundary and the shape rule is left for the genuinely unresolvable case
