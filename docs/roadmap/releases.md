...
- G233: an imported string-literal union prints as its literal set while an imported alias of the same declaration prints as its name, so one declaration has two spellings in a diagnostic and neither is what a local declaration prints. `imported_string_literal_union` lowers straight to `Ty::StringLiteralUnion`, which carries the literals and drops the name; the fix belongs with the lowering, since assignability decides both spellings correctly already. G75's rule, that a type keeps its name across a module boundary, is what this one lowering breaks
- G237: literal-typed expressions, so a string-literal union reached through a generic argument or a record field can be decided rather than fenced. D30 puts the literal set on the declaration, so `["read", "write"]` is `Array<string>` here and `Array<"read" | "write">` under `tsc`, and 0.1.122's rule declines that pairing rather than refusing it, which accepts a wrong program (`{ mode: "nope" }` against `{ mode: Mode }` is silent under `--no-tsc` and `TS2322` under `tsc`) to stop refusing a right one (`fn arr() -> Array<Mode> { return ["read", "write"] }`, which `tsc` compiles). The work is a literal type on a string literal, a join on an array literal, and a widening rule saying where it widens to `string`, which is inference rather than one relation
- G234: `glyph_impact`'s `change_signature_type` table sends a `Ty::StringLiteralUnion` to `Other` on both sides, so six cells say the checker has no rule for a pairing G230 gave it one for, and `glyph_assignable` on the same pairing says `COMPATIBLE`. This is G228's disagreement on a shape G228 did not cover, and the fix is the same: split `NamedBody::Other("a string-literal union (D30), which is a `string`")` out and decide each cell by the `assignability` call the container and record cells now make
