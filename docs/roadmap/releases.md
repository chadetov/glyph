...
- G101: **decided against `Step<A>`.** A new generic stdlib union would be the first of its kind and every one of the three facts above stands in its way: `match s { Continue(a) => a }` over a `Step<int>` would be a two-variant match with one arm and no diagnostic, which is G143 and G147 reintroduced through the stdlib door. What ships: `array.fold_while(xs, init, f: (acc: A, x: T) -> A, done: (acc: A) -> bool) -> A`, stopping before the next element when `done(acc)`, which the existing signature table expresses today, plus `array.try_fold(xs, init, f: (acc: A, x: T) -> Result<A, E>) -> Result<A, E>` over the prelude `Result` for the error-carrying case. Alpha-beta is `done = fn(acc) { acc.alpha >= acc.beta }`. A `Step` type, if still wanted, is a prelude proposal with its own review
