...
- **What the audit found (G214).** 49 rediscovery sites in six families; the emitter receives the `TypeMap` and consumes it in three places, and receives no cross-module type context because `ProjectTables::from_modules` runs over parse results before resolution. Three changes remove 30 of the 49 for almost no new surface, and they are this lane, in order:
  1. **A `decls: &dyn DeclTyResolver` field on `EmitContext`.** `build.rs` passes the `SalsaDeclTy` that `glyph-db` already constructs; the playground and the test harnesses pass the unit resolver that answers `None`, which is today's single-module behaviour exactly. No new salsa query: `exported_type`, `exported_fn`, `exported_const` and `decl_ty` already back the trait. This deletes five of the eight `ProjectTables` registries and `ModuleTypeNames`, and closes the open half of G147 by construction rather than by a second registry
  2. **The checker's `direct_type_decl`, `alias_target`, `resolve_alias_chain` and `split_type_app` become `pub`, and the emitter builds a `Lowerer` from the `resolved` and `prelude` it already holds.** Four visibility changes and one constructor call. The five alias walkers and six `direct_type_decl` copies go, the G207 refusal and the G212 hop read the same chain, and every `field_check` and `is_check` decision moves from `TypeExpr` spelling to `Ty`, which is G213's fix: a user `Option<T>` stops receiving the prelude's runtime check Change 1 landing found G224 on the checker's side of the same line: a local alias of an imported union is not exhaustiveness-checked, because `resolve_named_union` and `required_variants` never got the D46 hop. Change 2 gives them the hop, so one alias walker answers every question, in the checker and the emitter alike; breaking in the checking direction (a silently unchecked match goes red)
  3. **The checker's arm classification becomes the emitter's.** `classify_arm`, `is_catch_all_pattern` and `required_variants` are asked rather than mirrored, so the four `is_variant_shaped` rules collapse to one. This is the one change with a design edge: the emitter asks at spans the checker may not have classified (the temporaries `degroup_nested_arms` synthesises), so recording the classification per arm span into the `TypeMap` is the safer form and is the one taken
- G215, G226, G225: **pulled into this release.** Change 1 crossed the module boundary in the alias chain and thereby lost one true diagnostic (G226: a local record assigned to an alias of an imported union was `E0204`, now silent under `--no-tsc`), which is the same hole as G215 seen from the other side: `definitely_incompatible` has no rule for `Ty::Imported`. Both close here by resolving an imported declaration through `imported_type_decl` and deciding the pairing the way the local G201 rule does, so an imported union passed as a `string`, or assigned from a local record, is `E0211`/`E0204` under every surface. G225 is the namespace-import shadowing of `string` found by change 3; the checker reads a namespace binding as the prelude type it spells, the way the lowerer already does for `import std/result { Result }`. Breaking in the checking direction
- **What stays emitter-owned, on purpose.** Whether a declaration emits a runtime descriptor, its arity, the flat-versus-boxed payload representation, runtime verifiability (E0304), TypeScript-narrowing compensation, the `case`-label collision that made G147 a loud stop, and the hoisting analyses. These are facts about emission, not about the language, and the audit lists them so nobody spends a release moving them upstream. The one of them that should become a per-declaration `glyph-db` query rather than a whole-project eager scan is descriptor-ness, since sibling modules ask it
- G213: a user `type Option<T>` receives the prelude `Option`'s runtime check, so `Poll.parse` accepts `{"tag":"None"}` for a field that needs a `label`. Green build, wrong acceptance at the boundary; closed by change 2
...
**0.1.121 — One call describes a symbol, and the compiler stops passing what it should refuse**
- The first of the three priorities, measured by the audit's own probes: on a three-module project `glyph_hover` answered one position in fourteen, `glyph_variants` returned variant names without payloads, `glyph_symbols` returned ranges without signatures. Twenty-eight of the thirty-four items the audit found are facts the compiler already computes and does not hand over
- G218: **`glyph_symbol`**, one call, one `module::name` or a position, returning the symbol's kind, identity, visibility, fields with types, variants with payloads and the construction syntax of each (`Paid({ transaction_id: string })`), parameters and return, interface members, whether a match over it must be exhaustive, and its `@example` text. Built on `glyph_db::decl_ty`, `exported_fn`, `exported_const` and `display_ty`, which `glyph_impact` already reads for one verdict. `glyph_hover` answers at declaration names, parameters, annotations and variants by consulting the same query, and `glyph_definition` and `glyph_variants` carry the identity so an answer chains into `glyph_impact` without re-deriving it
- G219: `glyph_diagnostics` routes through the project database the other tools use and returns the `check --json` struct field for field, so the two surfaces cannot disagree. A gate asserts the shapes are one type
- G215, G216, G217: the three holes the audit fell into before it made a tool call, each green under every surface that runs without tsc. An imported union passed as a primitive (`imported_type_decl` is the query), a prelude application passed as a primitive (`Ty::App` against `Ty::Prim` in `definitely_incompatible`, including `Nullable<int>` against `int`, which tsc cannot catch because it narrows the emitted `number | null`), and a type name in expression position (`return Order { ... }`), which becomes a diagnostic naming the record-literal form rather than an unreachable-code warning. Breaking, each established by both binaries
- G221 (part): `glyph_assignable(from, to)` answers the question the checker asks itself, with `definitely_incompatible` exposed and the UNDETERMINED reading kept honest where the checker has no rule
- A CLI verb for each MCP query (`glyph query symbol ...`, `... impact ...`), so an agent without an MCP client has the same compiler
