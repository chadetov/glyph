...
- G212: **found by the review, scheduled here.** A resource reached through a D46 alias is `E0205` where the direct name is accepted; resource-ness follows the alias chain like everything else does. Breaking in the accepting direction only
- G211: a `tsc` error is mapped to the statement before the one that failed, so the caret an agent repairs from points at the wrong line
