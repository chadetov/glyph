...
- G203 and G206: **ruled, loosening only.** A `type` whose body is a bare path resolving to another `type` declaration is a second name for that declaration: `local_type_decl`, `named_record_fields` and `has_descriptor` follow the chain with the existing cycle guard, so `B` passes where `A` is declared, `b.naem` is `E0210` in one module as it already is across two, and `B.parse` resolves to `A.parse`. A body of any other form (record, union, string-literal union, `extern_ts`, `typeof`, function type, generic application, or anything carrying a `where`) is not followed, and no new incompatibility is decided from that fact: the corpus holds zero bare-path aliases and six `Record<string, T>` aliases, and a rule that made the six nominal would reject programs `tsc` accepts. The `change_signature_type` description in `mcp.rs` changes from "compared by name" to "compared by the declaration each name resolves to". Not breaking: it only accepts more
- G207: the `where`-over-alias divergence from D39 needs its ruling in this release, before the alias hop lands, since the hop makes the alias and the record one type. Ruling: D39's refusal is about the body's shape, so a `where` over an alias that resolves to a record or union is `E0300` like the direct spelling; the emitter's check moves from the syntactic body to the resolved one
- G205: an imported `const` is still `Ty::Unknown`. `glyph_db::exported_const`, the sibling of `exported_fn`, reads the annotation across the boundary and `imported_fn_decl` gets a const counterpart
