...
**0.1.124 — Next · What the benchmark runs on, closed**
...
bottom type, spellable in user code. Both are headings in
`docs/language/spec.md`, and `glyph llms --json` reports the collision in
`decisions.duplicate_numbers` rather than deduplicating it, which is how it was
found: the generator parses the spec's own list and two entries claim the same
number. Nothing in the compiler keys on a D number, so no program is affected.
What is affected is a reader following a citation, and both decisions are cited
by number in shipped release notes. Renumbering one of them is a decision about a
public index other documents point into, so it is the owner's call rather than a
cleanup: whichever one moves, the old number has to keep resolving somewhere.
