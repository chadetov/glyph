...
- **What this lane does.** An audit of `glyph-emit` lists every place a fact is re-derived from the AST or from a name's spelling (the `ProjectTables::scan` registries, the alias-leaf and alias-terminal walkers, the descriptor decisions read off a `TypeExpr`, the PascalCase shape rule for variant heads, the prelude and stdlib names matched by string), and for each names the resolver or checker function that already computes it. The findings are recorded as ledger entries and closed by making the emitter read the fact: through the context the driver already passes in where the fact exists, through a new `glyph-db` query where it does not. Where a fact is computed nowhere, it is added upstream. The `TypeMap` reaches the emitter or the reason it cannot is written down
