...
- G229: the parser has no nesting-depth guard, so 2,000 nested brackets abort `glyph check`, `glyph fmt`, `glyph lsp` and `glyph mcp` with a stack overflow; the scheduled fuzz workflow has failed on this every night since 2026-09-10. A depth limit over expressions, types and patterns raises a diagnostic at the span where it is crossed, and the two fuzz inputs become regression tests
