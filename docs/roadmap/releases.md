...
- **G230, a string-literal-union annotation refused nothing.** `type Mode = "read" | "write"` declares a finite set of strings (D30), and the checker compared a value against it by nothing at all, so a number, a `bool`, a record and an out-of-set literal all passed `check --no-tsc` while `tsc` refused every one. The hole was Glyph's own relation, which is the only thing `--no-tsc`, `glyph lsp`, `glyph mcp` and the playground have. The rule reads the union as what it is at run time, a `string`, and asks the relation the corresponding `string` question in each direction rather than re-deriving which values are records and which are containers. Two literal sets are compared by set, an alias chain is followed on both sides (D46), an imported union is decided like a local one, and `Nullable<T>` is descended with the union intact. 93 app and corpus entries were checked at their own roots before and after with identical exit codes and identical diagnostic-code lists
