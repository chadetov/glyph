...
- G105: **decided, with two signatures changed and one claim struck.** `std/fs` gains a handle reader: `open_lines(path) -> Result<LineReader, FsError>`, `next_line(r) -> Result<Option<string>, FsError>`, `close_lines(r) -> void`. `Option<string>` alone would report a disc error at line 400,000 as end of input, which is a silent truncation at a boundary. `next_line` closes the descriptor when it returns `Ok(None)`, and `close_lines` is idempotent, for the early stop. Synchronous, chunked `readSync` into a `StringDecoder` per handle, the way `io.read_line` already works on fd 0; the module header says it blocks and is not for a program that also serves. The shim gains `openSync` and `closeSync`, two declarations. The proposal's sentence "an `owned` resource so a dropped reader is E0206 like a socket" was false and is struck: no stdlib handle is tracked, `Socket` included. No new iteration construct: D21 has two loops and `for await` is a third, a spec amendment, and the streamed HTTP body has the same limit untouched. Not breaking
