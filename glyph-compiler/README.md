...
| `glyph-cli` | The `glyph` binary: `build [--no-tsc] [--no-test] [--json]`, `check [--no-tsc] [--json]`, `run`, `fmt`, `regen`, `gen` (openapi/dts/zod), `init`, `publish`, `lsp`, `mcp`, and `--explain`. |
