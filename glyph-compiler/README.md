...
| `glyph-lsp` | The language server and the MCP server, both over stdio, sharing one pure `analysis` layer (diagnostics, hover, definition, references, rename, symbols). The MCP server serves nine tools: `glyph_symbol`, `glyph_impact`, `glyph_assignable`, `glyph_diagnostics`, `glyph_hover`, `glyph_definition`, `glyph_references`, `glyph_variants`, `glyph_symbols`. `glyph_lsp::diagnostic::Diagnostic` is the JSON diagnostic both this crate and `glyph check --json` serialize. |
| `glyph-cli` | The `glyph` binary: `build [--no-tsc] [--no-test] [--json]`, `check [--no-tsc] [--json] [--agent]`, `run`, `fmt`, `fix`, `regen`, `gen` (openapi/dts/zod), `init`, `publish`, `lsp`, `--update`, `mcp`, `query <tool>` (one verb per MCP tool, printing the tool's JSON), `doctor`, `upgrade`, and `--explain`. It re-exports `glyph-lsp`'s `Diagnostic` rather than assembling its own, and reaches `glyph_symbol` through `glyph_lsp::call_mcp_tool` for `check --agent`'s symbol descriptions and `fix`'s variant payload shapes, so neither reads a declaration a second time. |
