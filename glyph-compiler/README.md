...
| `glyph-cli` | The `glyph` binary: `build [--no-tsc] [--no-test] [--json]`, `check [--no-tsc] [--json]`, `run`, `fmt`, `regen`, `gen` (openapi/dts/zod), `init`, `publish`, `lsp`, `--update`, `mcp`, `query <tool>` (one verb per MCP tool, printing the tool's JSON), `doctor`, `upgrade`, and `--explain`. It re-exports `glyph-lsp`'s `Diagnostic` rather than assembling its own. |
