//! The structured diagnostic every agent-facing surface answers with.
//!
//! One type, serialized by `glyph check --json` / `glyph build --json` and by
//! the `glyph_diagnostics` MCP tool. It used to be two: the CLI's eleven-field
//! struct here and a four-field object assembled inside the MCP server, with
//! `help` concatenated onto `message` and `severity`, `stage`, `file`, `union`
//! and `missing_variants` dropped. An agent reading both got two shapes for one
//! fact, which makes both untrustworthy (G219). The type lives in this crate
//! because `glyph-cli` depends on `glyph-lsp` and not the other way round, so
//! this is the lowest place both surfaces reach.
//!
...
//! An agent can then act on a batch of diagnostics by entity without
...
///
/// The walk itself is `enclosing_decl_name`, the one copy of the attribution
/// rule. Only the module half is assembled here, because only the caller knows
/// the root it is counted from: `module_path` arrives already stripped of the
/// project's `src` root by `build::derive_module_path` (G180), and by
/// `module_key` on the MCP side.
///
/// Without this, an agent acting on a diagnostic has to re-derive "which
/// function is this" from a line number, and a line number shifts under every
/// unrelated edit above the site, exactly the identity-loss problem the
/// `module::name` convention exists to prevent elsewhere.
pub fn entity_id(module_path: &str, module: &glyph_ast::Module, offset: u32) -> Option<String> {
    crate::enclosing_decl_name(module, offset).map(|name| format!("{module_path}::{name}"))
}

/// Map each `ResolveError` variant to the stage tag the diagnostic is reported
/// under. Stages let the reader tell a collect-time issue (a duplicate name)
/// from a resolve-time one (an unresolved name) from an import-time one (an
/// unknown export), and the advisory lint tier from all three.
///
/// One table, read by the JSON shape above and by the ariadne renderer in
/// `glyph-cli`, because a diagnostic that says `resolve` in the terminal and
/// `collect` in the JSON is two answers about one error.
...
/// Every diagnostic one file of a project produces, in the order and under the
/// stage tags `glyph check --json` reports them, read out of the incremental
/// database the project is already loaded into.
///
/// `module_path` is the file's module half, counted from its own project root
/// (D41), and it is what `file` and `entity` are spelled with. `tables` is the
/// project scan every emitted module needs, built once by the caller from the
/// same file set.
///
/// The order is the compiler's: parse, then the symbol table, then imports,
/// then resolution, then types, and the advisory lints and the emitter only on
/// a file that produced no errors, because there is no point linting or
/// emitting a program that does not check. That is `glyph build`'s rule, and
/// this list is held to it by a gate that runs both surfaces on one project
/// (`crates/glyph-cli/tests/agent_loop.rs`).
///
/// Two things `glyph check` does are not done here, both because they need
/// something outside the compiler:
///
/// - `tsc`. It runs over emitted TypeScript on disk, and this reads a project
///   without writing one.
/// - The missing-local-module check (`E0104`). Telling a mistyped sibling
///   import from an installed npm package needs the build's view of
///   `node_modules` and `package.json`; with no such view the resolver's own
///   answer is `Unknown`, and `Unknown` reports nothing.
pub fn project_file_diagnostics(
    db: &dyn Db,
    file: SourceFile,
    module_path: &str,
    tables: &ProjectTables,
) -> Vec<Diagnostic> {
    let source = file.source_text(db).clone();
    let mut out = Vec::new();

    let parsed = glyph_db::parse_module(db, file);
    let Some(ast) = parsed.module() else {
        // Downstream queries degrade to empty on a parse failure, so reporting
        // them would pile up cascade errors under one real one.
        if let Some(err) = parsed.error() {
            out.push(from_parse_error(module_path, &source, err));
        }
        return out;
    };
    ...
    for e in symbols.errors() {
        out.push(from_resolve_error(module_path, &source, e, ast));
    }

    for e in glyph_db::import_diagnostics(db, file).errors() {
        out.push(from_resolve_error(module_path, &source, e, ast));
    }

    // When collection failed, `resolve` hands back the collect errors verbatim,
    // so reporting them again doubles every duplicate-name and reserved-word
    // diagnostic.
    ...
    for e in types.errors() {
        out.push(from_type_error(module_path, &source, e, ast));
    }

    // Warnings do not block the tiers below; errors do.
    if out.iter().any(|d| d.severity == "error") {
        return out;
    }
    ...
    // The advisory tier: warnings that surface but never block emission.
    // Computed only on a module that resolved with no errors, so the
    // resolution map is complete and a used binding cannot be mistaken for a
    // dead one.
    for e in glyph_resolver::module_lints(ast, resolution) {
        out.push(from_resolve_error(module_path, &source, &e, ast));
    }
    if let Some(e) =
    ...
    // The emitter refuses some programs the checker accepts (a `?` in a
    // position it cannot hoist out of, two match arms that lower to one
    // `case`). Those are errors of `glyph check`, so they are errors here; the
    // emitted TypeScript itself is thrown away.
    let decls = glyph_db::SalsaDeclTy::new(db, file);
    let ctx = tables.emit_context(module_path, &decls);
    if let Err(e) = glyph_emit::emit_module_mapped(
        ast,
        resolution,
        types.type_map(),
        db.prelude(),
        ...
        out.push(from_emit_error(module_path, &source, &e, ast));
    }
...
mod tests {
    ...
    /// Regression guard for the has_catch_all bug shape (0.1.107): a
